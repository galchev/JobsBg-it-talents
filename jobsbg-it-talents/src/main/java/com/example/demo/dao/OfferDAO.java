...
public class OfferDAO {
	...
	private static final String SELECT_CITY_ID_WHERE_OFFER_ID =
			"SELECT c.city_id FROM offers o left join locations l on (o.location_id = l.location_id) left join cities c on (l.city_id = c.city_id)";
	private static final String SELECT_COUNTRY_ID_WHERE_OFFER_ID = 
			"SELECT c.country_id FROM offers o \r\n" + 
	...
	public List<SortedOfferDTO> getAllOffers(String sortBy,Long cityId,Long countryId,Long jobCategoryId,Long jobTypeId,Long jobLevelId,Long jobLanguageId,String keyword,Long companyId) throws SQLException{
		...
		SortedOfferDTO o = new SortedOfferDTO(rs.getLong(1), rs.getString(2),
				rs.getInt(3), rs.getDate(4), rs.getLong(5),rs.getLong(6),
				rs.getLong(7),rs.getLong(8), rs.getLong(9), rs.getLong(10),tempCityId,tempCountryId);
			offers.add(o);
		}
		...
		return offersToReturn.stream()
				.sorted((o1, o2) -> {
					...
					case "newest" : return o2.getDate().compareTo(o1.getDate());
					case "oldest" : return o1.getDate().compareTo(o2.getDate());
					default : return 1;
					}
				})
	...
	private List<SortedOfferDTO> sortOffersByCityId(List<SortedOfferDTO> offersToSort,Long cityId) throws SQLException{
		return offersToSort
				.stream()
				.filter(offer -> offer.getCityId().equals(cityId))
				.collect(Collectors.toList());
	}
	private List<SortedOfferDTO> sortOffersByCountryId(List<SortedOfferDTO> offersToSort,Long countryId) throws SQLException{
		return offersToSort
				.stream()
				.filter(offer -> offer.getCountryId().equals(countryId))
				.collect(Collectors.toList());
	}
	private List<SortedOfferDTO> sortOffersByJobLevelId(List<SortedOfferDTO> offersToSort,Long jobLevelId) throws SQLException{
		return offersToSort
				.stream()
				.filter(offer -> offer.getJobLevelId().equals(jobLevelId))
				.collect(Collectors.toList());
	}
	private List<SortedOfferDTO> sortOffersByJobLanguageId(List<SortedOfferDTO> offersToSort,Long jobLanguageId) throws SQLException{
		return offersToSort
				.stream()
				.filter(offer -> offer.getJobLanguageId().equals(jobLanguageId))
				.collect(Collectors.toList());
	}
	private List<SortedOfferDTO> sortOffersByJobCategoryId(List<SortedOfferDTO> offersToSort,Long jobCategoryId) throws SQLException{
		return offersToSort
				.stream()
				.filter(offer -> offer.getJobCategoryId().equals(jobCategoryId))
				.collect(Collectors.toList());
	}
	private List<SortedOfferDTO> sortOffersByJobTypeId(List<SortedOfferDTO> offersToSort,Long jobTypeId) throws SQLException{
		return offersToSort
				.stream()
				.filter(offer -> offer.getJobTypeId().equals(jobTypeId))
				.collect(Collectors.toList());
	}
	...
	private List<SortedOfferDTO> sortOffersByCompanyId(List<SortedOfferDTO> offersToSort,Long companyId) throws SQLException{
		return offersToSort
				.stream()
				.filter(offer -> offer.getCompanyRegId().equals(companyId))
				.collect(Collectors.toList());
	}
